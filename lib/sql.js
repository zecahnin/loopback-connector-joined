...
SQLConnector.prototype._buildWhere = function(model, where) {
  ...
  for (var key in where) {
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        for (var i = 0, n = clauses.length; i < n; i++) {
          for (var keyC in clauses[i]) {
            if(keyC.indexOf('.')!=-1){
              var subQuery = keyC.split('.')
              var model = self.getModelDefinition(model)
                  .model.definition.settings.relations[subQuery[0]].model;
              clauses[i][subQuery[1]]=clauses[i][keyC];
              delete clauses[i][keyC];
            }
          }
    ...
    var columnName = self.tableEscaped(model)+'.'+self.columnEscaped(model, key);
...
SQLConnector.prototype.buildOrderBy = function(model, order) {
  ...
  for (var i = 0, n = order.length; i < n; i++) {
    ...
    if (t.length === 1) {
      clauses.push(self.tableEscaped(model)+'.'+self.columnEscaped(model, order[i]));
    } else {
      clauses.push(self.tableEscaped(model)+'.'+self.columnEscaped(model, t[0]) + ' ' + t[1]);
    }
  }
...
SQLConnector.prototype.buildColumnNames = function(model, filter) {
  ...
  if (Array.isArray(fieldsFilter) && fieldsFilter.length > 0) {
    ...
    keys = fieldsFilter.filter(function(f) {
      return self.tableEscaped(model)+'.'+cols[f];
    });
  ...
  var names = keys.map(function(c) {
    return self.tableEscaped(model)+'.'+self.columnEscaped(model, c);
  });
  return names.join(',');
};
...
SQLConnector.prototype.buildSelect = function(model, filter, options) {
  var self = this;
  var groupBy = false;
  var whereJoinStmt=null;
  var orderJoinStmt=null;
  var whereJoin=false;
  if (!filter.order) {
  ...
  if (filter) {
    if (filter && filter.include && self.getMetadata().isRelational()) {
      for (var inc in filter.include) {
        var objectRelation=null;
        var props={};
        if (typeof filter.include[inc] != 'function') {
          if (typeof filter.include[inc] === 'object') {
            objectRelation=filter.include[inc].relation;
            props=filter.include[inc];
            if(!props.scope){
              props.scope={};
            }
          }else{
            objectRelation=filter.include[inc];
            if(!props.scope){
              props.scope={};
            }
          }
          var relations = self.getModelDefinition(model)
              .model.definition.settings.relations[objectRelation];
          if ((typeof props !== "undefined" && props !== null ? props.scope.joined : void 0) != null){
            if(props.scope && props.scope.where){
              whereJoinStmt = self.buildWhere(relations.model, props.scope.where);
              whereJoin=true;
            }
            if(props.scope && props.scope.order){
              orderJoinStmt=self.buildOrderBy(relations.model, props.scope.order);
            }
            var joinStmt = self.buildJoin(model, objectRelation, props,filter);
            groupBy = true;
    ...
    if (filter.where) {
      ...
      if(whereJoinStmt){
        var sqlsWhere=[];
        var paramsWhere=[];
        sqlsWhere.push(whereStmt.sql.replace('WHERE',''));
        sqlsWhere.push(whereJoinStmt.sql.replace('WHERE',''));
        paramsWhere.push(whereStmt.params);
        paramsWhere.push(whereJoinStmt.params);
        whereStmt = new ParameterizedSQL({
          sql: ' WHERE '+sqlsWhere.join(' AND '),
          params: paramsWhere,
        });
      }
      ...
      if(whereJoinStmt){
        selectStmt.merge(whereJoinStmt);
      }
    }
    if(groupBy){
      var groupByStmt = ' GROUP BY ' + self.buildColumnNames(model, filter);
      selectStmt.merge(groupByStmt);
    }
    if (filter.order) {
      if(orderJoinStmt){
        selectStmt.merge(orderJoinStmt);
      }else{
...
/**
 * Build Join query to database
 *
 *
 * @param {String} model The model name
 * @param {String} objectRelation the relations of table
 * @param {Object} props proprieties data from element
 * @param {Object} filter The filter
 */

SQLConnector.prototype.buildJoin = function(model, objectRelation,props,filter) {
  var self = this;
  var JoinClause = new ParameterizedSQL('');
  var idDefault = self.idColumn(model);
  var relations = self.getModelDefinition(model)
      .model.definition.settings.relations[objectRelation];
  if(props.scope.joined) {
    var firstrelationOn='';
    var secondtrelationOn='';
    if(relations.type==='belongsTo'){
      relations.foreignKey=relations.foreignKey?relations.foreignKey:(objectRelation+'Id');
      firstrelationOn=relations.foreignKey;
      secondtrelationOn=idDefault;
    }
    if(relations.type==='hasMany'){
      relations.foreignKey=relations.foreignKey?relations.foreignKey:(model+'Id');
      firstrelationOn=idDefault;
      secondtrelationOn=relations.foreignKey;
      if(relations.through){
        JoinClause.sql = props.scope.joined + ' JOIN ' + this.tableEscaped(relations.through)
            + ' as ' + self.tableEscaped(relations.through) + ' ON ('
            + self.tableEscaped(model) + '.'
            + (firstrelationOn)
            + ' = '
            + self.tableEscaped(relations.through) + '.'
            + (secondtrelationOn) + ')';
        secondtrelationOn=idDefault;

        var relationsThrough = self.getModelDefinition(relations.model)
            .model.definition.settings.relations[model.toCamelCase()];
        model=relations.through;
        firstrelationOn=relationsThrough.foreignKey;
      }
    }

    JoinClause.sql += props.scope.joined + ' JOIN ' + this.tableEscaped(relations.model)
        + ' as ' + self.tableEscaped(relations.model) + ' ON ('
        + self.tableEscaped(model) + '.'
        + (firstrelationOn)
        + ' = '
        + self.tableEscaped(relations.model) + '.'
        + (secondtrelationOn) + ')';
  }
  return JoinClause;
};
